...
func (evm *EVM) Call(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error, op_count map[string]int64, op_time map[string]int64, op_time_list map[string][]int64, op_gas_list map[string][]uint64) {
	// Fail if we're trying to execute above the call depth limit
	if evm.depth > int(params.CallCreateDepth) {
		return nil, gas, ErrDepth, nil, nil, nil, nil
	}
	// Fail if we're trying to transfer more than the available balance
	if !value.IsZero() && !evm.Context.CanTransfer(evm.StateDB, caller.Address(), value) {
		return nil, gas, ErrInsufficientBalance, nil, nil, nil, nil
	}
	...
	if !evm.StateDB.Exist(addr) {
		if !isPrecompile && evm.chainRules.IsEIP158 && value.IsZero() {
			...
			return nil, gas, nil, nil, nil, nil, nil
		}
	...
	if isPrecompile {
		...
		if len(code) == 0 {
			...
			ret, err, op_count, op_time, op_time_list, op_gas_list = evm.interpreter.Run(contract, input, false)
			gas = contract.Gas
		}
	}
	...
	return ret, gas, err, op_count, op_time, op_time_list, op_gas_list
}
...
func (evm *EVM) CallCode(caller ContractRef, addr common.Address, input []byte, gas uint64, value *uint256.Int) (ret []byte, leftOverGas uint64, err error) {
	...
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		...
		ret, err, _, _, _, _ = evm.interpreter.Run(contract, input, false)
		gas = contract.Gas
	}
	if err != nil {
		evm.StateDB.RevertToSnapshot(snapshot)
...
func (evm *EVM) StaticCall(caller ContractRef, addr common.Address, input []byte, gas uint64) (ret []byte, leftOverGas uint64, err error) {
	...
	if p, isPrecompile := evm.precompile(addr); isPrecompile {
		...
		ret, err, _, _, _, _ = evm.interpreter.Run(contract, input, true)
		gas = contract.Gas
	}
	if err != nil {
		evm.StateDB.RevertToSnapshot(snapshot)
...
func (evm *EVM) create(caller ContractRef, codeAndHash *codeAndHash, gas uint64, value *uint256.Int, address common.Address, typ OpCode) ([]byte, common.Address, uint64, error) {
	...
	ret, err, _, _, _, _ := evm.interpreter.Run(contract, nil, false)
