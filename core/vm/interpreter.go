...
func (in *EVMInterpreter) Run(contract *Contract, input []byte, readOnly bool) (ret []byte, err error, op_count map[string]int64, op_time map[string]int64, op_time_list map[string][]int64, op_gas_list map[string][]uint64) {
	...
	for {
		...
		fmt.Println("pc is ", pc, " .Opcode name is", op.String(), "Before Error!!")

		operation := in.table[op]
		cost = operation.constantGas // For tracing
		// Validate stack
		if sLen := stack.len(); sLen < operation.minStack {
			return nil, &ErrStackUnderflow{stackLen: sLen, required: operation.minStack}, nil, nil, nil, nil
		...
		if operation.dynamicGas != nil {
			...
			if operation.memorySize != nil {
				...
				if overflow {
					return nil, ErrGasUintOverflow, nil, nil, nil, nil
				}
